...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    $hab = [];
    $sql2 = "SELECT h.id as haid,  h.nom as namehab from habitats h where 1";
   $res = $connect->query($sql2);
   if($res ){
  $hab = $res->fetch_all(MYSQLI_ASSOC);
   }
}
...
<aside class="w-72 bg-white dark:bg-slate-900 border-r border-slate-100 dark:border-slate-800 flex-col hidden md:flex h-screen sticky top-0 shrink-0">
    ...
    <section class="p-6 bg-surface-light dark:bg-surface-dark rounded-xl shadow-xl border border-gray-100 dark:border-gray-800">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input value="<?= $animal['nom']??'' ?>" type="text" name="nom" required class="w-full px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-background-light dark:bg-surface-dark/50 text-text-light dark:text-text-dark" placeholder="Ex: Simba" />
            </div>
    ...
    <input type="text" 
           ...
           class="w-full px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-background-light dark:bg-surface-dark/50 text-text-light dark:text-text-dark" 
           placeholder="Ex: Carnivore" />
</div>
...
<div>
    <label class="block text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark mb-1">
        Habitat Affecté <span class="text-red-500">*</span>
    </label>
   <select name="id_habitat" required class="w-full px-4 py-2 border rounded-lg dark:bg-transparent">
                                    <option value="" disabled <?= !$animal ? 'selected' : '' ?>>Sélectionner un habitat</option>
                                    <?php foreach ($hab as $h) : ?>
                                        <option value="<?= $h['haid'] ?>" <?= ($animal && $h['haid'] == $animal['id_habitat']) ? 'selected' : '' ?>>
                                            <?= $h['namehab'] ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
